...
import { redirectIfNotLoggedIn, isInstructor, getUser, logout } from './auth.js';
...
function initNavMenu() {
  ...
  if (me) {
    ...
    if (avatar) avatar.textContent = ((me.first_name?.[0]||'')+(me.last_name?.[0]||'')).toUpperCase() || 'U';
  }
  document.addEventListener('click', (e) => {
    const dropdown = document.getElementById('userDropdown');
    if (dropdown && !e.target.closest('.nav-user-wrap')) dropdown.classList.remove('open');
  });
...
function initAutoSave() {
  ...
  document.getElementById('previewBtn')?.addEventListener('click', () => {
    saveDraft();
    showToast('Draft saved. A live preview becomes available once the course is published.', 'info');
  });
}
...
function collectCurriculum() {
  ...
  document.querySelectorAll('.section-block').forEach(block => {
    const title = block.querySelector('.section-input')?.value?.trim() || 'Untitled section';
    const lessons = [];
    block.querySelectorAll('.lesson-row').forEach(row => {
      const lessonTitle = row.querySelector('.lesson-title-input')?.value?.trim();
      const typeLabel = row.querySelector('.lesson-type-select')?.value || '▶ Video';
      if (lessonTitle) {
        const type = typeLabel.includes('Quiz') ? 'quiz' : typeLabel.includes('Article') ? 'article' : 'video';
        lessons.push({ title: lessonTitle, type });
      }
    });
    if (lessons.length) sections.push({ title, lessons });
  });
  return sections;
}
...
export async function publishCourse() {
  ...
  try {
    ...
    const created = await instructorApi.createCourse(courseData);

    // Send each curriculum lesson to the new course (best-effort; course still gets created if this fails)
    const sections = collectCurriculum();
    for (const section of sections) {
      for (const lesson of section.lessons) {
        try {
          await instructorApi.addLesson(created.id, { title: `${section.title}: ${lesson.title}`, type: lesson.type });
        } catch (err) { /* continue with remaining lessons */ }
      }
    }

    localStorage.removeItem('lh_course_draft');
    showToast('Course submitted and published! 🎉', 'success');
